...
h_pc = 10.                  # m    porous column height; 0 < h_pc <= h_l
w_pc = 0.15                 #      porous column porosity
k_pc = 1.22e-13             # m2   porous column permeability
...
MonotonicQl, gamma
                  ...
                  "Example_2_SAMBA_User_Guide.txt")       # Export results in text file
...
plot(t, 1.e-6*DP_c, label = r'$P_{C}$')
plot(t, 1.e-6*DP_lb, label = r'$P_{L-}$')
ylabel(r'$P-P(t=0)\ (MPa)$')
legend(loc = 'best')
xlabel('$\mathrm{Time}\  (years)$')
savefig('Example_2_SAMBA_User_Guide_FlowRate&Pressures.png', dpi = 200)
...
legend(loc = 'best')
xlabel('$\mathrm{Time}\  (years)$')
savefig("Example_2_SAMBA_User_Guide_Drinving&CounteringPressures.png", format = 'png', dpi = 200)
...
savefig("Example_2_SAMBA_User_Guide_ConvolutionIntegralsApproximation.png", format = 'png', dpi = 200)
